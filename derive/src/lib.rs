...
use syn::{DeriveInput, parse_macro_input, Data, Fields, token, Type, Field};
...
pub fn derive_builder(input: TokenStream) -> TokenStream {
    ...
    let input_ident = input.ident;  // 获取类名
    ...
    if let Data::Struct(r) = input.data {   // 处理
        ...
        // 获取builder的属性值
        let builder_lets = map_fields(&fields, &mut |(ident, ty)| {
            quote!(
                let #ident = self.#ident.ok_or(format!(
                    "field {:?}  not set yes", stringify!(#ident),
                ))?;
            )
        });

        // 初始化时的默认值
        let builder_fields_values = map_fields(&fields, &mut |(ident, ty)| {
            quote!(
                #ident,
            )
        });
